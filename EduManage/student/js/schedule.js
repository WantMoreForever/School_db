...
const API = window.studentGetApiUrl('schedule');
    ...
    function scheduleEmptyEl(){
        let empty = document.getElementById('scheduleEmptyState');
        if (!empty) {
            empty = document.createElement('div');
            empty.id = 'scheduleEmptyState';
            empty.className = 'no-schedule fade-up delay-2';
            const page = document.getElementById('page-schedule');
            if (page) page.appendChild(empty);
        }
        return empty;
    }
    function showScheduleEmpty(title, subtitle){
        const empty = scheduleEmptyEl();
        if (!empty) return;
        empty.innerHTML = '<div class="ns-icon">📅</div><div class="ns-title">'+escapeHtml(title)+'</div><div class="ns-sub">'+escapeHtml(subtitle)+'</div>';
        empty.style.display = '';
    }
    function hideScheduleEmpty(){
    ...
    function initScheduleData() {
        ...
        const noStudentNotice = document.getElementById('noStudentNotice');

        if (noStudentNotice) noStudentNotice.style.display = 'none';
        hideScheduleEmpty();
        if (timetable) timetable.innerHTML = '';
        if (overviewList) overviewList.innerHTML = '';
        if (semesterBanner) semesterBanner.style.display = 'none';
        if (scheduleToolbar) scheduleToolbar.style.display = 'none';
        ...
        fetch(API, { credentials: 'include' }).then(r=>r.json()).then(resp=>{
            if(!resp || !resp.ok){
                const msg = (window.StudentApi && window.StudentApi.messageOf) ? window.StudentApi.messageOf(resp, '课程表加载失败') : ((resp && resp.message) || '课程表加载失败');
                console.error('API error', resp);
                showScheduleAlert('error', msg);
                showScheduleEmpty('课程表加载失败', msg);
                return;
            }
            ...
            if(!hasSchedules){
                ...
                if (semesterBanner && data.semester_label) semesterBanner.style.display = '';
                showScheduleEmpty('暂无课程表', '当前学期还没有已选且已排课的课程。');
                return;
            }
            ...
            showScheduleAlert('error', '课程表加载失败，请稍后重试');
            showScheduleEmpty('课程表加载失败', '请求未完成，请稍后重试。');
        });
    }
