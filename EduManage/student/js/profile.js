...
(function () {
    var studentApi = window.StudentApi || {};
    ...
    function resetAvatarForm() {
        ...
        if (avatarSubmit) {
            if (studentApi.setButtonBusy) {
                studentApi.setButtonBusy(avatarSubmit, false);
            }
            avatarSubmit.disabled = true;
        }
        if (avatarForm) {
            delete avatarForm.dataset.submitting;
        }
        if (avatarPreviewRow) {
    ...
    function bindAvatarUi() {
        ...
        if (avatarForm && avatarInput) {
            avatarForm.addEventListener('submit', function (ev) {
                ev.preventDefault();
                if (avatarForm.dataset.submitting === '1') {
                    return;
                }
                ...
                if (avatarSubmit) {
                    if (studentApi.setButtonBusy) {
                        studentApi.setButtonBusy(avatarSubmit, true, '上传中...');
                    } else {
                        avatarSubmit.disabled = true;
                    }
                }
                avatarForm.dataset.submitting = '1';
    ...
    function bindInfoForm() {
        ...
        infoForm.addEventListener('submit', function (ev) {
            ev.preventDefault();
            if (infoForm.dataset.submitting === '1') {
                return;
            }
            ...
            if (submitBtn) {
                if (studentApi.setButtonBusy) {
                    studentApi.setButtonBusy(submitBtn, true, '保存中...');
                } else {
                    submitBtn.disabled = true;
                }
            }
            infoForm.dataset.submitting = '1';
            ...
            fetchProfileJson({
                ...
                if (submitBtn) {
                    if (studentApi.setButtonBusy) {
                        studentApi.setButtonBusy(submitBtn, false);
                    } else {
