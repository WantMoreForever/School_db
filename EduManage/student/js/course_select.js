(function(){
    ...
    function captureCsrf(payload) {
        ...
        const token = payload.csrf_token || data.csrf_token;
    ...
    async function apiFetch(method='GET', body=null){
        ...
        if (body && method.toUpperCase() === 'POST') {
            ...
            let csrfToken = '';
            try {
                csrfToken = window && window.__CSRF_TOKEN ? window.__CSRF_TOKEN : '';
    ...
    function renderOverview(data){
        ...
        const sections = Array.isArray(data.available_sections) ? data.available_sections : [];
        const availEl = document.getElementById('availableCount');
        if (availEl) availEl.textContent = String(sections.filter(sec => Number(sec.can_enroll) === 1).length);
    ...
    function renderAvailable(data){
        ...
        (data.available_sections||[]).forEach(sec=>{
            ...
            const nameNotes = [];
            if (Number(sec.is_restricted) === 1) {
                nameNotes.push(`<div style="font-size:12px;color:${Number(sec.is_major_allowed) === 1 ? '#475467' : '#b42318'};margin-top:4px;font-weight:500;">${Number(sec.is_major_allowed) === 1 ? '专业限选' : '专业限制'}</div>`);
            }
            if (Number(sec.conflict_flag) === 1 && Number(sec.is_my_course) === 0) {
                nameNotes.push(`<div style="font-size:12px;color:#b42318;margin-top:4px;font-weight:500;">时间冲突</div>`);
            }
            tdName.innerHTML = `<div style="font-weight:500;margin-bottom:2px;">${escapeHtml(sec.course_name||'')}</div>` + schedHtml + nameNotes.join('');
            ...
            } else if (Number(sec.can_enroll) !== 1) {
                let reason = sec.disabled_reason || '不可选';
                if (reason === '不在选课时间') {
                    reason = windowState === 'not_started' ? '未开始' : '已截止';
                }
                actionTd.innerHTML = `<button class="cs-btn-disabled" disabled>${escapeHtml(reason)}</button>`;
            } else {
