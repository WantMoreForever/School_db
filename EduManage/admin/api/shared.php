...
function admin_api_json_envelope(bool $ok, array $payload = [], int $status = 200): array
{
    ...
    $code = (string) ($payload['code'] ?? ($ok ? app_config('api.default_success_code', 'OK') : admin_api_error_code_for_status($status)));
...
{
    $unsafeActs = app_config('auth.admin_unsafe_actions', []);
    if (!is_array($unsafeActs)) {
        $unsafeActs = [];
    }
...
function admin_api_handle_avatar_upload()
{
    ...
    $maxSize = (int) app_config('upload.avatar.max_size', 0);
    if ($maxSize > 0 && $file['size'] > $maxSize) {
        return ['error' => '头像文件过大，最大 ' . number_format($maxSize / 1024 / 1024, 0) . 'MB'];
    }

    $finfo = new finfo(FILEINFO_MIME_TYPE);
    $mime = $finfo->file($file['tmp_name']);
    $allowed = app_config('upload.avatar.allowed_mimes', []);
    if (!is_array($allowed)) {
        $allowed = [];
    }
...
{
    $targets = app_config('enums.announcement_targets', []);
    $globalTargets = array_intersect(array_keys(is_array($targets) ? $targets : []), ['all', 'students', 'teachers']);
    if (in_array($target, $globalTargets, true)) {
        return [$target, 0];
    }
