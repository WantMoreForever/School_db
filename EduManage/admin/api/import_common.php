...
function admin_import_json_envelope(bool $ok, array $payload = [], int $status = 200): array
{
    ...
    $code = (string) ($payload['code'] ?? ($ok ? 'OK' : admin_import_error_code_for_status($status)));
...
function admin_import_read_rows_from_upload(?array $file): array
{
    ...
    if ($ext === 'xlsx') {
        return admin_import_read_xlsx_rows($tmpName);
    }
    if ($ext === 'csv') {
        return admin_import_read_csv_rows($tmpName);
    }

    throw new RuntimeException('仅支持 .xlsx 或 .csv 文件');
}
