...
app_start_session();
$htmlContentType = (string) app_config('api.html_content_type', '');
if ($htmlContentType !== '') {
    header('Content-Type: ' . $htmlContentType);
}
$timezone = (string) app_config('app.timezone', '');
if ($timezone !== '') {
...
function admin_is_super_admin(): bool
{
    ...
    try {
        ...
        return app_is_super_admin_role($role);
    } catch (Throwable $e) {
...
function admin_school_email_domain(): string
{
    $domain = strtolower(trim((string) app_config('auth.school_email_domain', 'school.edu')));
    return $domain !== '' ? $domain : 'school.edu';
}

function admin_school_email_regex(): string
{
    return '/^[A-Za-z0-9._%+-]+@' . preg_quote(admin_school_email_domain(), '/') . '$/i';
}

function admin_school_email_pattern(): string
{
    return '[A-Za-z0-9._%+-]+@' . preg_quote(admin_school_email_domain(), '/');
}

function admin_school_email_title(): string
{
    return '请输入以 @' . admin_school_email_domain() . ' 结尾的邮箱';
}

function admin_validate_school_email(string $email, string $fieldLabel = '邮箱', ?string $emptyMessage = null): ?string
{
    if ($email === '') {
        return $emptyMessage ?? ($fieldLabel . '不能为空');
    }

    if (!preg_match(admin_school_email_regex(), $email)) {
        return $fieldLabel . '必须是以 @' . admin_school_email_domain() . ' 结尾的有效地址';
    }
