...
function admin_manage_validate_school_email(string $email): ?string
{
    if ($email === '') {
        return '请输入用户邮箱';
    }
    if (!preg_match('/^[A-Za-z0-9._%+-]+@school\.edu$/', $email)) {
        return '管理员邮箱必须是以 @school.edu 结尾的有效地址';
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'add') {
            ...
            if ($msg = admin_manage_validate_school_email($email)) throw new Exception($msg);
...
$stmt = $pdo->prepare('SELECT a.user_id, a.role, u.name, u.email FROM admin a LEFT JOIN user u ON a.user_id = u.user_id WHERE a.user_id != ? AND a.role != "super_admin" ORDER BY a.user_id DESC');
...
<div class="card mb-3">
    <div class="card-body">
        <form id="addAdminForm" class="row g-2">
            ...
            <div class="col-auto">
                <input name="email" class="form-control" placeholder="用户邮箱" required inputmode="email" pattern="[A-Za-z0-9._%+-]+@school\.edu" title="请输入以 @school.edu 结尾的邮箱">
            </div>
            <div class="col-auto">
