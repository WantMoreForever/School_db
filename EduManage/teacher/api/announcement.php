...
function action_get_section(PDO $pdo, int $teacherId): void
{
    ...
    try {
        $rows = function_exists('app_call_rows')
            ? app_call_rows($pdo, 'sp_project_get_section_announcements', [$sectionId, $teacherId])
            : [];
    } catch (Throwable $e) {
        $sql = "SELECT a.announcement_id, a.author_user_id, u.name AS teacher_name, a.title, a.content,
                       a.is_pinned, a.published_at, a.created_at, a.updated_at,
                       (a.author_user_id = ?) AS is_author
                FROM announcement a
                JOIN announcement_target at ON at.announcement_id = a.announcement_id
                JOIN user u ON u.user_id = a.author_user_id
                WHERE at.target_type = 'section' AND at.target_id = ?
...
function action_get_teacher(PDO $pdo, int $teacherId): void
{
    try {
        $rows = function_exists('app_call_rows')
            ? app_call_rows($pdo, 'sp_project_get_author_announcements', [$teacherId])
            : [];
    } catch (Throwable $e) {
        $sql = "SELECT a.announcement_id, a.author_user_id, a.title, a.content, a.status, a.is_pinned,
                       a.published_at, a.created_at, a.updated_at, u.name AS teacher_name,
                       1 AS is_author,
...
function action_get_inbox(PDO $pdo, int $teacherId): void
{
    ...
    $where = "a.status = 'published' AND (
                (at.target_type = 'section' AND at.target_id IN (
                    SELECT section_id FROM teaching WHERE teacher_id = ?
                ))
    ...
    try {
        $sets = function_exists('app_call_multi_result_rows')
            ? app_call_multi_result_rows($pdo, 'sp_project_get_teacher_visible_announcements', [$teacherId, $q, $perPage, $offset])
            : [];
