...
try {
    ...
    teacher_handle_exception($e, 'teacher.api.schedule');
} catch (Throwable $e) {
    teacher_handle_exception($e, 'teacher.api.schedule');
}
 ...
 * Body: schedule_id, day_of_week, start_time, end_time, classroom_id
 */
function action_update_schedule(PDO $pdo, int $tid): void {
    ...
    $classroom_id = (int)($body['classroom_id'] ?? 0);

    if (!$schedule_id || !$day_of_week || !$start_time || !$end_time || !$classroom_id) {
        json_err('Missing required fields: schedule_id, day_of_week, start_time, end_time, classroom_id');
    }
    ...
    $stmt->execute([$schedule_id, $day_of_week, $start_time, $end_time, $classroom_id, $tid]);
    $stmt->nextRowset();
    ...
    sys_log($pdo, $tid, sys_log_build('更新课程安排', [
        'teacher_id' => $tid,
 ...
 * Body: section_id, day_of_week, start_time, end_time, classroom_id, week_start, week_end
 */
function action_add_schedule(PDO $pdo, int $tid): void {
    ...
    $end_time    = trim($body['end_time'] ?? '');
    $classroom_id = (int)($body['classroom_id'] ?? 0);
    $week_start  = (int)($body['week_start'] ?? 1);
    $week_end    = (int)($body['week_end'] ?? 16);

    if (!$section_id || !$day_of_week || !$start_time || !$end_time || !$classroom_id) {
        json_err('Missing required fields: section_id, day_of_week, start_time, end_time, classroom_id');
    }
    ...
    $stmt->execute([$section_id, $day_of_week, $start_time, $end_time, $classroom_id, $week_start, $week_end, $tid]);
    $stmt->nextRowset();
    ...
    if (!$success) json_err($message, 400);
    sys_log($pdo, $tid, sys_log_build('新增课程安排', [
        'teacher_id' => $tid,
        'schedule_id' => $schedule_id,
    ]), 'schedule', $schedule_id);
...
function action_delete_schedule(PDO $pdo, int $tid): void {
    ...
    if (!$success) json_err($message, 400);
    sys_log($pdo, $tid, sys_log_build('删除课程安排', [
        'teacher_id' => $tid,
        'schedule_id' => $schedule_id,
    ]), 'schedule', $schedule_id);
    json_ok(['message' => $message]);
...
/**
 * GET /api/schedule.php?action=check_conflicts&section_id=X&day_of_week=Y&start_time=Z&end_time=W&classroom_id=C
...
function action_check_conflicts(PDO $pdo, int $tid): void {
    ...
    $room_conflict->execute([$classroom_id, $day_of_week, $start_time, $end_time]);
