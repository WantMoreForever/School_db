...
function app_is_production_env(): bool
{
    $env = strtolower((string) app_config('app.env', 'local'));
...
function app_session_cookie_options(): array
{
    $config = app_config('app.session.cookie', []);
    if (!is_array($config)) {
        $config = [];
    }
    ...
    $sameSite = trim((string) ($config['samesite'] ?? 'Lax'));
    if ($sameSite === '') {
        $sameSite = 'Lax';
    }
    ...
    if (!empty($config['secure_when_https']) && app_request_is_https()) {
        $secure = true;
    }
    if (!empty($config['secure_in_production']) && app_is_production_env()) {
        $secure = true;
    }
...
function app_configure_session_cookie(): void
{
    if (headers_sent()) {
        return;
    }
...
function app_session_numeric_value(string $key): ?int
{
    app_start_session();
...
{
    return app_session_numeric_value('user_id');
}

function app_current_teacher_id(PDO $pdo): ?int
{
    $teacherId = app_session_numeric_value('teacher_id');
    if ($teacherId !== null) {
...
function app_current_teacher_profile(PDO $pdo, ?int $teacherId = null): ?array
{
    $teacherId = $teacherId ?? app_current_teacher_id($pdo);
    if ($teacherId === null) {
        return null;
    }
    ...
    $row['user_id'] = (int) ($row['user_id'] ?? $teacherId);
    $row['name'] = $name !== '' ? $name : ($email !== '' ? $email : '教师用户');
    $row['email'] = $email;
    $row['title'] = trim((string) ($row['title'] ?? '')) !== '' ? (string) $row['title'] : '教师';
    $row['dept_name'] = (string) ($row['dept_name'] ?? '');
    $row['dept_code'] = (string) ($row['dept_code'] ?? '');
    $row['avatar_url'] = $image !== '' ? app_avatar_url() . $image : app_default_avatar_url();
...
function app_logout_and_redirect(?string $path = null): never
{
    ...
    if (ini_get('session.use_cookies')) {
        $params = app_session_cookie_options();
