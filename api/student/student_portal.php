...
if ($pdo && !$db_error) {
    ...
    // ──────────────────────────────────────────────────────────────────────
    // 【可配置项】以下两个变量建议从系统配置表读取，例如：
    //   SELECT cfg_value FROM sys_config WHERE cfg_key = 'recent_grades_limit'
    //   SELECT cfg_value FROM sys_config WHERE cfg_key = 'recent_grades_year_range'
    $recent_grades_limit      = 8;   // 最多显示条数；对应配置键: recent_grades_limit
    $recent_grades_year_range = 2;   // 显示近几年（含今年）；对应配置键: recent_grades_year_range
    // ──────────────────────────────────────────────────────────────────────

    $current_year = (int)date('Y');
    $year_min     = $current_year - ($recent_grades_year_range - 1); // 例：2026-1 = 2025，即显示2025和2026
    ...
    $stmt = $pdo->prepare("
        ...
        LIMIT ?
    ");
    $stmt->execute([$uid, $year_min, $recent_grades_limit]);
    $recent_grades = $stmt->fetchAll();

    // ──────────────────────────────────────────────────────────────────────
    // 【可配置项】选课窗口时间，建议从系统配置表读取，例如：
    //   SELECT cfg_value FROM sys_config WHERE cfg_key = 'enroll_start'
    //   SELECT cfg_value FROM sys_config WHERE cfg_key = 'enroll_end'
    $enroll_start = '2024-09-01 08:00:00';  // 对应配置键: enroll_start
    $enroll_end   = '2024-09-07 23:59:59';  // 对应配置键: enroll_end
    // ──────────────────────────────────────────────────────────────────────
        ...
        <div class="bottom-row fade-up delay-4">
            ...
            <div class="grade-card">
                <div class="card-header">
                    <div class="card-header-title">📊 最近成绩
                        <span style="font-size:11px;font-weight:400;color:var(--ink-muted);margin-left:6px;">
                            <?= $year_min ?> 年起 
                        </span>
                    </div>
                ...
                <?php if (empty($recent_grades)): ?>
                    <div class="empty-state"><div class="es-icon">📭</div>近两年暂无已公布成绩</div>
                <?php else: ?>
            ...
            <div class="info-card">
                <div class="info-avatar-section">
                    ...
                    <a class="avatar-edit-link" href="profile.php#avatar">✏ 修改个人信息</a>
                </div>
                <div class="info-list">
