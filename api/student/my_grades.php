...
if ($pdo) {
    ...
    foreach ($rows as $row) {
        // 【改动】仅期末类型计算绩点，其余置 null；表格中将显示"—"
        // 【可配置项】'final' 对应 sys_config: gpa_exam_type
        $row['gpa_point'] = ($row['exam_type'] === 'final') ? scoreToGpa($row['score']) : null;
        $grades[] = $row;
    }
...
// 【改动】GPA 汇总仅统计 exam_type = 'final' 的行
// 【可配置项】'final' 对应 sys_config: gpa_exam_type
$total_cx_gpa = 0.0;
$total_credit = 0.0;
$final_count  = 0;
    ...
    <div class="content">
        ...
        <div class="gpa-hero fade-up">
            ...
            <div class="gpa-details">
                ...
                <!-- 【改动】说明文字标注仅期末成绩计入 GPA -->
                <div class="gpa-sub">加权 GPA = Σ(学分 × 绩点) ÷ 总学分 · 仅期末成绩计入 · 随学期筛选联动</div>
                <div class="gpa-tags">
                    <div class="gpa-tag">
                    ...
                    <!-- 【改动】区分显示期末条数与全部条数 -->
                    <div class="gpa-tag">
                        <span class="gt-val"><?= $final_count ?></span>
                        <span class="gt-label">期末成绩数</span>
                    </div>
                    <div class="gpa-tag">
                        <span class="gt-val"><?= count($grades) ?></span>
                        <span class="gt-label">全部成绩数</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grades-list fade-up delay-3">
            <?php foreach ($grades as $g):
                $gp      = $g['gpa_point'];   // null = 非 final，不计入 GPA
                $sc      = (float)$g['score'];
                $scColor = scoreColor($sc);
                $gpColor = ($gp !== null) ? gpaColor((float)$gp) : 'var(--ink-muted)';
            ?>
            <div class="grade-row">
                <div>
                ...
                <div class="col-center">
                    <?php if ($gp !== null): ?>
                        ...
                        <span class="gpa-chip" style="color:<?= $gpColor ?>;border-color:<?= $gpColor ?>44;background:<?= $gpColor ?>18">
                            <?= number_format((float)$gp, 1) ?>
                        </span>
                    <?php else: ?>
                        <!-- 【改动】非期末成绩不计入 GPA，显示"—"并附 tooltip -->
                        <span class="gpa-chip" style="color:var(--ink-muted);border-color:var(--ink-muted)44;background:var(--ink-muted)18"
                              title="仅期末成绩计入 GPA">—</span>
                    <?php endif; ?>
                </div>
            </div>
            <?php endforeach; ?>
        ...
        <div class="gpa-summary-bar fade-up delay-4">
            <span>
                ...
                期末 <strong><?= $final_count ?></strong> 条参与 GPA ·
